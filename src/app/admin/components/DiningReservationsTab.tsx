...
import {
  ...
  UtensilsCrossed, Clock, X, CalendarRange, LayoutList, Archive, ArchiveRestore,
} from 'lucide-react';
...
export default function DiningReservationsTab({ diningReservations, deleteDiningReservation, archivedDiningReservations, onArchivePast }: DiningReservationsTabProps) {
  ...
  const pagination = usePagination({ data: filteredDining, itemsPerPage: 10 });
  ...
  const hasPast = diningReservations.some((r) => r.date < todayStr);
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="inline-flex rounded-xl border border-hotel-200 dark:border-dark-border bg-white dark:bg-dark-card p-1 shadow-sm">
          <button onClick={() => setArchiveView(false)} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${!archiveView ? 'bg-gold-600 text-white shadow-sm' : 'text-hotel-500 dark:text-hotel-400 hover:text-hotel-700 dark:hover:text-hotel-200'}`}>
            <UtensilsCrossed className="h-3.5 w-3.5" />Active ({diningReservations.length})
          </button>
          <button onClick={() => setArchiveView(true)} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${archiveView ? 'bg-gold-600 text-white shadow-sm' : 'text-hotel-500 dark:text-hotel-400 hover:text-hotel-700 dark:hover:text-hotel-200'}`}>
            <Archive className="h-3.5 w-3.5" />Archived ({archivedDiningReservations.length})
          </button>
        </div>
      ...
      {archiveView && (
        <>
          {archivedDiningReservations.length === 0 ? (
            <div className={`${cardCls} p-16 text-center`}>
              ...
              <p className="text-hotel-500 dark:text-hotel-400">Past dining reservations will appear here after archiving.</p>
            </div>
          ) : (
        ...
        {viewMode === 'list' && <div>
          ...
          {filteredDining.length === 0 ? (
            ...
            <div className="space-y-4">
              {pagination.paginatedData.map((res) => {

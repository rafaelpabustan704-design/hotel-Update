...
import type { RoomDetail, ManagedRoom, RoomType } from '@/utils/types';
...
function RoomModal({ room, onClose, onBookNow }: RoomModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-5xl max-h-[90vh] rounded-2xl bg-white dark:bg-dark-card shadow-2xl animate-modal-content overflow-y-auto">
        ...
        <div className="grid lg:grid-cols-2 lg:h-full lg:max-h-[90vh]">
          ...
          <div className="p-5 sm:p-6 lg:p-7 lg:overflow-y-auto lg:max-h-[90vh]">
            <div className="mb-4">
              ...
              <h2 className="font-serif text-2xl font-bold text-hotel-900 dark:text-white mb-1">{room.name}</h2>
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-hotel-500 dark:text-hotel-400 leading-relaxed">{room.longDescription}</p>
            </div>
            <div className="mb-4">
            ...
            <button onClick={() => { onClose(); onBookNow(room.roomTypeKey); }} className="w-full rounded-xl bg-gold-600 py-3 text-sm font-bold uppercase tracking-wider text-white shadow-lg shadow-gold-600/25 transition-all hover:bg-gold-700 hover:shadow-gold-700/30 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400 focus-visible:ring-offset-2">
...
export default function Rooms({ onBookNow }: RoomsProps) {
  ...
  const mergedRooms = useMemo(() => {
    ...
    const fromManaged: RoomDetail[] = managedRooms.map((mr) => {
      ...
      const staticMatch = ROOMS.find((s) => s.roomTypeKey === rtName);

      const features = mr.amenities
        .map((a) => AMENITY_TO_FEATURE[a.toLowerCase()])
        .filter((f): f is string => !!f);

      const specs = [
        { icon: BedDouble, label: 'Bed', value: mr.bedType ? `${mr.bedQty || 1} ${mr.bedType}${mr.extraBedType ? ` + ${mr.extraBedQty || 1} ${mr.extraBedType}` : ''}` : (staticMatch?.specs.find((s) => s.label === 'Bed')?.value) || 'King Size' },
        { icon: Ruler, label: 'Size', value: mr.roomSize || (staticMatch?.specs.find((s) => s.label === 'Size')?.value) || '' },
        { icon: Eye, label: 'View', value: mr.view || (staticMatch?.specs.find((s) => s.label === 'View')?.value) || '' },
      ...
      if (staticMatch) {
        ...
        return {
          ...
          id: mr.id,
          name: mr.name,
          tagline: mr.tagline || staticMatch.tagline,
          description: mr.description || staticMatch.description,
          longDescription: mr.longDescription || staticMatch.longDescription,
          price: mr.price > 0 ? mr.price : staticMatch.price,
          images: mr.images.length > 0 ? mr.images : staticMatch.images,
          features: features.length > 0 ? features : staticMatch.features,
          specs,
          inclusions: mr.inclusions?.length > 0 ? mr.inclusions : staticMatch.inclusions,
        };
      }
      ...
      return {
        ...
        longDescription: mr.longDescription || mr.description || `Enjoy your stay in our ${mr.name.toLowerCase()}. This room accommodates up to ${mr.maxPax} guests and includes ${mr.amenities.join(', ') || 'essential amenities'}.`,
        price: mr.price,
        ...
        features,
        specs,
        inclusions: mr.inclusions?.length > 0 ? mr.inclusions : (mr.amenities.length > 0 ? mr.amenities : ['Daily housekeeping', 'High-speed WiFi']),
      };
    });

    const unmatchedStatic = ROOMS.filter((s) => !matchedStaticIds.has(s.id));

    return [...fromManaged, ...unmatchedStatic];
    ...
    <section id="rooms" className="scroll-mt-20 py-24 bg-hotel-50 dark:bg-dark-bg transition-colors">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-gold-600 mb-3">Accommodations</p>
          <h2 className="font-serif text-4xl font-bold text-hotel-900 dark:text-white mb-4">Our Signature Rooms</h2>
